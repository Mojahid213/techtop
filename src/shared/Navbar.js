                ...
                <ul id="Menu_list" className={scrolled ? "hidden md:flex md:flex-1 gap-8 md:justify-end items-center text-center text-white uppercase font-Signika transition duration-500" : "hidden md:flex md:flex-1 gap-8 md:justify-end items-center text-center text-black uppercase font-Signika"}>
                    ...
                    {JSON.parse(sessionStorage.getItem('token')) ?
                        <li className="mt-4 md:mt-0 relative">
                            <button id="profile_pic" className={scrolled ? "h-10 w-10 bg-white text-black text-base font-RobotoSlab rounded-full p-1.5 border-2 border-gray-400 hover:border-opacity-0 ring-indigo-400 hover:ring-4 transition duration-300" : "h-10 w-10 bg-black text-white text-base font-RobotoSlab rounded-full p-1.5 border-2 border-gray-400 hover:border-opacity-0 ring-indigo-400 hover:ring-4 transition duration-300"}>{user.avatar}</button>
                            <ul id="profile_card" className="hidden absolute right-1/2 md:right-0 -mr-16 md:-mr-0 mt-3 bg-white border shadow-lg rounded-lg text-black p-3 w-52 md:w-60">
                                <div className="cursor-pointer flex justify-center items-center h-16 w-16 md:h-20 md:w-20 bg-black text-white font-RobotoSlab rounded-full p-1.5 mx-auto"><span className="text-4xl md:text-5xl">{user.avatar}</span></div>
                                ...
                                <li className="mt-4 cursor-pointer text-left"><p className="normal-case hover:text-blue-700"><FontAwesomeIcon icon={faSignOutAlt} /> Logout</p></li>
                            </ul>
                        </li>
